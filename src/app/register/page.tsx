...
function RegisterContent() {
  ...
  async function handleEmail(e: React.FormEvent) {
    ...
    if (error) {
      ...
      setMessage('¡Revisá tu email! Te enviamos un link mágico para crear tu cuenta.')
    }
  }
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-white flex items-center justify-center p-6">
      <div className="w-full max-w-sm">
        ...
        <h1 className="text-2xl font-bold text-center mb-2">Creá tu cuenta</h1>
        <p className="text-zinc-400 text-center text-sm mb-8">
          Tu pieza está lista 🎉 Creá tu cuenta para descargarla
        </p>
        ...
        <p className="text-center text-sm text-zinc-500 mt-6">
          ¿Ya tenés cuenta?{' '}
          <Link href={`/login?redirectTo=${encodeURIComponent(redirectTo)}`} className="text-[#CCFF90] hover:underline">
            Entrar
          </Link>
        </p>
      </div>
